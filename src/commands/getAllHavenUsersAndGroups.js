...
const extractInfoFromGroupName = (groupName) => {
  const groupRegExp = /HavenSecrets(?<project>.*)(?<env>(Dev|Prod|Stg))(?<permission>.*)Group/;
...
const getAllHavenUsersAndGroups = async () => {
  try {
    ...
    const filteredUsers = havenUsers.Users.filter(
      (userData) => !/HavenSecretsTemporaryUser/.test(userData.UserName)
    );
    const havenIam = createHavenIam();
    const groupPromises = filteredUsers.map(
      async (user) =>
